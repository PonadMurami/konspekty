"""Sphinx extension to create tags for documentation pages.

"""

import os
import re
from pathlib import Path
from typing import Dict, List, Optional, Tuple

from docutils import nodes
from sphinx.errors import ExtensionError
from sphinx.util.docutils import SphinxDirective
from sphinx.util.logging import getLogger
from sphinx.util.matching import get_matching_files
from sphinx.util.rst import textwidth

__version__ = "0.4"

logger = getLogger("sphinx-tags")

CATEGORY_SEPARATOR = "|"

_DIR_TITLE_CACHE: Dict[str, str] = {}


def _read_first_title_line(lines: List[str], suffix: str) -> str:
    """Best-effort title extraction for a source document."""
    for line in lines:
        s = line.strip()
        if not s:
            continue
        # Skip the tags directive if it appears before a title (rare).
        if s.startswith(".. tags::") or s.startswith("```{tags}"):
            continue
        if suffix == ".md" and s.startswith("#"):
            return s.lstrip("#").strip()
        return s
    return ""


def _get_dir_title(srcdir: Path, dir_name: str) -> str:
    """Get human-friendly cycle/parent title from <dir>/index.rst if present."""
    if not dir_name:
        return ""
    if dir_name in _DIR_TITLE_CACHE:
        return _DIR_TITLE_CACHE[dir_name]

    index_path = srcdir / dir_name / "index.rst"
    title = ""
    if index_path.exists():
        try:
            title = _read_first_title_line(
                index_path.read_text(encoding="utf-8").splitlines(), ".rst"
            )
        except Exception:
            title = ""

    title = title or dir_name
    _DIR_TITLE_CACHE[dir_name] = title
    return title


def _split_categorized_tag(raw_tag: str) -> Tuple[Optional[str], str, str]:
    """
    Parse tag in format:
      - "kategoria|nazwa_taga|tekst_do_wyswietlenia"
      - "kategoria|nazwa_taga" (no display text; display falls back to tag name)
      - "nazwa_taga" (no category; display falls back to tag name)

    Returns: (category_or_none, tag_name, display_text)

    Category is lower-cased and stripped. Tag name and display text are stripped.
    """
    raw_tag = _normalize_display_tag(raw_tag)
    if CATEGORY_SEPARATOR not in raw_tag:
        return None, raw_tag, raw_tag

    parts = raw_tag.split(CATEGORY_SEPARATOR, 2)
    if len(parts) == 2:
        category, name = parts
        display = name
    else:
        category, name, display = parts[0], parts[1], parts[2]
    category = _normalize_display_tag(category).lower()
    name = _normalize_display_tag(name)
    display = _normalize_display_tag(display)
    if not display:
        display = name
    return (category or None), name, display


def _on_config_inited(app, config) -> None:
    # Load allowed categories (and their order) from config tags_badge_colors keys.
    # The order of keys in conf.py is preserved and used for display sorting.
    badge_colors = getattr(config, "tags_badge_colors", {}) or {}
    category_order: List[str] = []
    if isinstance(badge_colors, dict):
        for k in badge_colors.keys():
            if not isinstance(k, str):
                continue
            if k == "*":
                continue
            kk = k.strip().lower()
            if kk:
                category_order.append(kk)

    config.tags_known_categories = set(category_order)
    config.tags_category_order = {cat: i for i, cat in enumerate(category_order)}

    # Avoid duplicate labels when old generated tag dirs are present.
    # We always keep ONLY the active `tags_output_dir` in the doc set.
    active = str(getattr(config, "tags_output_dir", "_tags")).strip().strip("/\\")
    exclude = list(getattr(config, "exclude_patterns", []) or [])

    # Detect other tag-output dirs by presence of generated index file.
    # This avoids hardcoding names (e.g. "tag", "tagi", "_tags").
    srcdir = Path(app.srcdir)
    for p in srcdir.iterdir():
        if not p.is_dir():
            continue
        if p.name == active:
            continue
        if (p / "tagsindex.rst").exists() or (p / "tagsindex.md").exists():
            pat = f"{p.name}/**"
            if pat not in exclude:
                exclude.append(pat)

    config.exclude_patterns = exclude


def _warn(app, docname: str, lineno: int, message: str) -> None:
    # Use Sphinx location formatting (file:line).
    logger.warning(message, location=(docname, lineno))


class TagLinks(SphinxDirective):
    """Custom directive for adding tags to Sphinx-generated files.

    Loosely based on https://stackoverflow.com/questions/18146107/how-to-add-blog-style-tags-in-restructuredtext-with-sphinx

    See also https://docutils.sourceforge.io/docs/howto/rst-directives.html

    This directive can be used with arguments and with content.

    1. With arguments:

        .. raw:: rst
            .. tags:: tag1, tag2, tag3

    2. With (multiline) content:

        .. raw:: rst
            .. tags::

                tag1, tag2,
                tag3
    """

    # Sphinx directive class attributes
    required_arguments = 0
    optional_arguments = 1  # Arbitrary, split on separator
    final_argument_whitespace = True
    has_content = True
    final_argument_whitespace = True
    # Custom attributes
    separator = ","

    def run(self):
        if not (self.arguments or self.content):
            raise ExtensionError("No tags passed to 'tags' directive.")

        raw_tags: List[str] = []
        # normalize white space and remove "\n"
        if self.arguments:
            raw_tags.extend(
                [_normalize_display_tag(tag) for tag in self.arguments[0].split(",")]
            )
        if self.content:
            # self.content: StringList(['different, tags,', 'separated'],
            #                          items=[(path, lineno), (path, lineno)])
            raw_tags.extend(
                [
                    _normalize_display_tag(tag)
                    for tag in ",".join(self.content).split(",")
                ]
            )

        # Parse categorized tags and remove empties
        # Spec: (category_or_none, tag_id, display_text)
        tag_specs: List[Tuple[Optional[str], str, str]] = []
        for raw in raw_tags:
            if not raw:
                continue
            category, name, display = _split_categorized_tag(raw)
            if not name:
                continue
            tag_specs.append((category, name, display))

        tag_dir = Path(self.env.app.srcdir) / self.env.app.config.tags_output_dir
        result = nodes.paragraph()
        result["classes"] = ["tags"]
        result += nodes.inline(text=f"{self.env.app.config.tags_intro_text} ")
        count = 0

        current_doc_dir = Path(self.env.doc2path(self.env.docname)).parent
        relative_tag_dir = Path(os.path.relpath(tag_dir, current_doc_dir))

        known_categories = getattr(self.env.app.config, "tags_known_categories", set()) or set()
        category_order = getattr(self.env.app.config, "tags_category_order", {}) or {}

        # Display tags grouped in the order defined in conf.py (tags_badge_colors keys).
        if category_order:
            tag_specs = sorted(
                tag_specs,
                key=lambda x: category_order.get((x[0] or "").strip().lower(), 10_000),
            )

        for category, name, display in tag_specs:
            count += 1
            # We want the link to be the path to the _tags folder, relative to
            # this document's path where
            #
            #  - self.env.app.config.tags_output_dir
            # |
            #  - subfolder
            #   |
            #    - current_doc_path

            file_basename = _normalize_tag(name, dashes=True)

            # Validation / warnings
            if category is None:
                if category_order:
                    opts = ", ".join(
                        [f"'{c}{CATEGORY_SEPARATOR}'" for c in category_order.keys()]
                    )
                    msg = (
                        f"sphinx-tags: tag '{name}' is missing category prefix; expected one of: {opts}"
                    )
                else:
                    msg = f"sphinx-tags: tag '{name}' is missing category prefix"

                _warn(self.env.app, self.env.docname, self.lineno, msg)
            else:
                if known_categories and category.strip().lower() not in known_categories:
                    if category_order:
                        opts = ", ".join(category_order.keys())
                        msg = (
                            f"sphinx-tags: unknown tag category '{category}' for tag '{name}' (expected one of: {opts})"
                        )
                    else:
                        msg = f"sphinx-tags: unknown tag category '{category}' for tag '{name}'"

                    _warn(self.env.app, self.env.docname, self.lineno, msg)

            if self.env.app.config.tags_create_badges:
                result += self._get_badge_node(
                    display, file_basename, relative_tag_dir, category
                )
            else:
                result += self._get_plaintext_node(display, file_basename)
            if not count == len(tag_specs):
                if self.env.app.config.tags_create_badges:
                    sep = nodes.inline(text=", ")
                    sep["classes"] = ["tags-sep-badge"]
                    result += sep
                else:
                    result += nodes.inline(text=f"{self.separator} ")

        # register tags to global metadata for document
        self.env.metadata[self.env.docname]["tags"] = [name for _, name, _ in tag_specs]

        return [result]

    def _get_plaintext_node(self, tag: str, file_basename: str) -> List[nodes.Node]:
        """Get a plaintext reference link for the given tag"""
        link = Path(self.env.app.config.tags_output_dir) / f"{file_basename}/"
        return nodes.reference(refuri="/" + str(link), text=tag)

    def _get_badge_node(
        self,
        tag: str,
        file_basename: str,
        relative_tag_dir: Path,
        category: Optional[str],
    ) -> List[nodes.Node]:
        """Get a sphinx-design reference badge for the given tag"""
        from sphinx_design.badges_buttons import XRefBadgeRole

        # Required to set Inliner state, since we're directly creating a role object.
        # Typically this would be done when parsing the role from document text.
        text_nodes, messages = self.state.inline_text("", self.lineno)

        # Ref paths always use forward slashes, even on Windows
        tag_ref = f"{tag} <{relative_tag_dir.as_posix()}/{file_basename}>"
        tag_color = self._get_tag_color(category=category)
        role_name_color = tag_color if tag_color is not None else "plain"
        tag_badge = XRefBadgeRole(tag_color)
        return tag_badge(
            name=f"bdg-ref-{role_name_color}",
            rawtext=tag,
            text=tag_ref,
            lineno=self.lineno,
            inliner=self.state.inliner,
        )[0]

    def _get_tag_color(self, category: Optional[str]) -> Optional[str]:
        """
        Select badge color by category.

        `tags_badge_colors` must be a dict where keys are categories (from conf.py).

        If category is missing/unknown, use '*' if provided, otherwise 'primary'.
        """
        tag_colors = self.env.app.config.tags_badge_colors or {}

        if category:
            key = category.strip().lower()
            if key in tag_colors:
                return tag_colors[key]

        if "*" in tag_colors:
            return tag_colors["*"]

        return "primary"


class Tag:
    """A tag contains entries"""

    def __init__(
        self,
        tag: str,
        display_name: Optional[str] = None,
        category: Optional[str] = None,
    ):
        self.items = []
        # `tag` is the stable id used for filenames/URLs.
        self.tag = _normalize_display_tag(tag)
        # `name` is what we show in tag overview/pages.
        self.name = _normalize_display_tag(display_name or tag)
        # `category` is used for category overview pages.
        self.category = _normalize_display_tag(category).lower() if category else None
        self.file_basename = _normalize_tag(self.tag, dashes=True)

    def create_file(
        self,
        items,
        extension,
        tags_output_dir,
        srcdir,
        tags_page_title,
        tags_page_header,
    ):
        """Create file with list of documents associated with a given tag in
        toctree format.

        This file is reached as a link from the tag name in each documentation
        file, or from the tag overview page.

        If we are using md files, generate and md file; otherwise, go with rst.

        Parameters
        ----------

        tags_output_dir : Path
            path where the file for this tag will be created
        items : list
            list of files associated with this tag (instance of Entry)
        extension : {["rst"], ["md"], ["rst", "md"]}
            list of file extensions used.
        srcdir : str
            root folder for the documentation (usually, project/docs)
        tags_page_title: str
            the title of the tag page, after which the tag is listed (e.g. "Tag: programming")
        tags_page_header: str
            the words after which the pages with the tag are listed (e.g. "With this tag: Hello World")
        tag_intro_text: str
            the words after which the tags of a given page are listed (e.g. "Tags: programming, python")


        """
        # Group entries by parent (cycle) title for readability.
        # Structure: {parent_title: [(entry_title, relpath), ...]}
        groups: Dict[str, List[Tuple[str, str]]] = {}
        for e in items:
            rel = e.relpath(srcdir)
            entry_title = getattr(e, "entry_title", "") or rel
            parent_title = getattr(e, "parent_title", "") or "Inne"
            groups.setdefault(parent_title, []).append((entry_title, rel))

        grouped_entries: List[Tuple[str, List[Tuple[str, str]]]] = []
        for parent_title in sorted(groups.keys(), key=lambda s: s.casefold()):
            entries = sorted(groups[parent_title], key=lambda x: (x[0].casefold(), x[1]))
            grouped_entries.append((parent_title, entries))

        ref_label = f"sphx_tag_{self.file_basename}"

        content = []
        if "md" in extension:
            filename = f"{self.file_basename}.md"
            content.append(f"({ref_label})=")
            content.append(f"# {tags_page_title}: {self.name}")
            content.append("")
            if tags_page_header:
                content.append(f"**{tags_page_header}**")
                content.append("")
            for parent_title, entries in grouped_entries:
                content.append("```{toctree}")
                content.append("---")
                content.append(f"caption: {parent_title}")
                content.append("maxdepth: 1")
                content.append("---")
                for entry_title, rel in entries:
                    content.append(f"{entry_title} <../{rel}>")
                content.append("```")
                content.append("")
        else:
            filename = f"{self.file_basename}.rst"
            header = f"{tags_page_title}: {self.name}"
            content.append(f".. _{ref_label}:")
            content.append("")
            content.append(header)
            content.append("#" * textwidth(header))
            content.append("")
            if tags_page_header:
                content.append(f".. rubric:: {tags_page_header}")
                content.append("")
            for parent_title, entries in grouped_entries:
                content.append(".. toctree::")
                content.append("    :maxdepth: 1")
                content.append(f"    :caption: {parent_title}")
                content.append("")
                for entry_title, rel in entries:
                    content.append(f"    {entry_title} <../{rel}>")
                content.append("")

        content.append("")
        with open(
            os.path.join(srcdir, tags_output_dir, filename), "w", encoding="utf8"
        ) as f:
            f.write("\n".join(content))


class Entry:
    """Tags to pages map"""

    def __init__(self, entrypath: Path, srcdir: Optional[str] = None):
        self.filepath = entrypath
        # self.tags = tags
        # Read tags (for the first time) to create the tag pages
        self.lines = self.filepath.read_text(encoding="utf8").split("\n")
        self.entry_title = _read_first_title_line(self.lines, self.filepath.suffix)

        self.parent_title = ""
        if srcdir:
            try:
                rel = self.filepath.relative_to(Path(srcdir))
                if len(rel.parts) >= 2:
                    parent_dir = rel.parts[0]
                    self.parent_title = _get_dir_title(Path(srcdir), parent_dir)
            except Exception:
                self.parent_title = ""

        if self.filepath.suffix == ".rst":
            tagstart = ".. tags::"
            tagend = ""  # empty line
        elif self.filepath.suffix == ".md":
            tagstart = "```{tags}"
            tagend = "```"
        elif self.filepath.suffix == ".ipynb":
            tagstart = '".. tags::'
            tagend = "]"
        else:
            raise ValueError(
                "Unknown file extension. Currently, only .rst, .md .ipynb are supported."
            )

        # tagline = [line for line in self.lines if tagstart in line]
        # tagblock is all content until the next new empty line
        tagblock = []
        reading = False
        for line in self.lines:
            line = line.strip()
            if tagstart in line:
                reading = True
                line = line.split(tagstart)[1]
                tagblock.extend(line.split(","))
            else:
                if reading and line == tagend:
                    # tagblock now contains at least one tag
                    if tagblock != [""]:
                        break
                if reading:
                    tagblock.extend(line.split(","))

        self.tags = []
        self.tag_display: Dict[str, str] = {}
        self.tag_category: Dict[str, str] = {}
        if tagblock:
            parsed: List[str] = []
            for raw in tagblock:
                raw = _normalize_display_tag(raw)
                if not raw:
                    continue
                category, name, display = _split_categorized_tag(raw)
                if name:
                    parsed.append(name)
                    if name not in self.tag_display:
                        self.tag_display[name] = display
                    if category and name not in self.tag_category:
                        self.tag_category[name] = category
            self.tags = parsed

    def assign_to_tags(self, tag_dict):
        """Append ourself to tags"""
        for tag in self.tags:
            if tag not in tag_dict:
                tag_dict[tag] = Tag(
                    tag,
                    self.tag_display.get(tag),
                    self.tag_category.get(tag),
                )
            else:
                # If we now have a better display name, enrich existing tag.
                display = self.tag_display.get(tag)
                if display and getattr(tag_dict[tag], "name", None) == getattr(tag_dict[tag], "tag", tag):
                    tag_dict[tag].name = _normalize_display_tag(display)
                # If we now have a category, enrich existing tag.
                category = self.tag_category.get(tag)
                if category and not getattr(tag_dict[tag], "category", None):
                    tag_dict[tag].category = _normalize_display_tag(category).lower()
            tag_dict[tag].items.append(self)

    def relpath(self, root_dir) -> str:
        """Get this entry's path relative to the given root directory"""
        return Path(os.path.relpath(self.filepath, root_dir)).as_posix()


def _normalize_tag(tag: str, dashes: bool = False) -> str:
    """Normalize a tag name to use in output filenames and tag URLs.
    Replace whitespace and other non-alphanumeric characters with dashes.

    Example: 'Tag:with (special   characters) ' -> 'tag-with-special-characters'
    """
    char = " "
    if dashes:
        char = "-"
    return re.sub(r"[\s\W]+", char, tag).lower().strip(char)


def _normalize_display_tag(tag: str) -> str:
    """Strip extra whitespace from a tag name for display purposes.

    Example: '  Tag:with (extra   whitespace) ' -> 'Tag:with (extra whitespace)'
    """
    tag = tag.replace("\\n", "\n").strip('"').strip()
    return re.sub(r"\s+", " ", tag)


def tagpage(tags, outdir, title, extension, tags_index_head):
    """Creates Tag overview page.

    This page contains a list of all available tags.

    """

    tags = list(tags.values())

    if "md" in extension:
        content = []
        content.append("---")
        content.append("orphan: true")
        content.append("---")
        content.append("")
        content.append("(tagoverview)=")
        content.append("")
        content.append(f"# {title}")
        content.append("")
        # toctree for this page
        content.append("```{toctree}")
        content.append("---")
        content.append(f"caption: {tags_index_head}")
        content.append("maxdepth: 1")
        content.append("---")
        for tag in sorted(tags, key=lambda t: t.name):
            content.append(f"{tag.name} ({len(tag.items)}) <{tag.file_basename}>")
        content.append("```")
        content.append("")
        filename = os.path.join(outdir, "tagsindex.md")
    else:
        content = []
        content.append(":orphan:")
        content.append("")
        content.append(".. _tagoverview:")
        content.append("")
        content.append(title)
        content.append("#" * textwidth(title))
        content.append("")
        # toctree for the page
        content.append(".. toctree::")
        content.append(f"    :caption: {tags_index_head}")
        content.append("    :maxdepth: 1")
        content.append("")
        for tag in sorted(tags, key=lambda t: t.name):
            content.append(
                f"    {tag.name} ({len(tag.items)}) <{tag.file_basename}.rst>"
            )
        content.append("")
        filename = os.path.join(outdir, "tagsindex.rst")

    with open(filename, "w", encoding="utf8") as f:
        f.write("\n".join(content))


def tag_category_page(
    tags: List[Tag],
    outdir: str,
    title: str,
    extension,
    tags_index_head: str,
    filename_base: str,
    page_title_template: str = None,
):
    """Creates a category-specific tag overview page (filtered tag list)."""

    tags = list(tags)

    if page_title_template is None:
        page_title_template = "Tagi w kategorii: {category}"
    page_title = page_title_template.format(category=title)

    if "md" in extension:
        content = []
        content.append("---")
        content.append("orphan: true")
        content.append("---")
        content.append("")
        content.append(f"({filename_base})=")
        content.append("")
        content.append(f"# {page_title}")
        content.append("")
        content.append("```{toctree}")
        content.append("---")
        content.append("maxdepth: 1")
        content.append("---")
        for tag in sorted(tags, key=lambda t: t.name):
            content.append(f"{tag.name} ({len(tag.items)}) <{tag.file_basename}>")
        content.append("```")
        content.append("")
        filename = os.path.join(outdir, f"{filename_base}.md")
    else:
        content = []
        content.append(":orphan:")
        content.append("")
        content.append(page_title)
        content.append("#" * textwidth(page_title))
        content.append("")
        content.append(".. toctree::")
        content.append("    :maxdepth: 1")
        content.append("")
        for tag in sorted(tags, key=lambda t: t.name):
            content.append(f"    {tag.name} ({len(tag.items)}) <{tag.file_basename}.rst>")
        content.append("")
        filename = os.path.join(outdir, f"{filename_base}.rst")

    with open(filename, "w", encoding="utf8") as f:
        f.write("\n".join(content))


def assign_entries(app):
    """Assign all found entries to their tag."""
    pages = []
    tags = {}

    # Get document paths in the project that match specified file extensions
    doc_paths = get_matching_files(
        app.srcdir,
        include_patterns=[f"**.{extension}" for extension in app.config.tags_extension],
        exclude_patterns=app.config.exclude_patterns,
    )

    for path in doc_paths:
        entry = Entry(Path(app.srcdir) / path, srcdir=app.srcdir)
        entry.assign_to_tags(tags)
        pages.append(entry)

    return tags, pages


def update_tags(app):
    """Update tags according to pages found"""
    if app.config.tags_create_tags:
        tags_output_dir = Path(app.config.tags_output_dir)

        if not os.path.exists(os.path.join(app.srcdir, tags_output_dir)):
            os.makedirs(os.path.join(app.srcdir, tags_output_dir))

        for file in os.listdir(os.path.join(app.srcdir, tags_output_dir)):
            if file.endswith("md") or file.endswith("rst"):
                os.remove(os.path.join(app.srcdir, tags_output_dir, file))

        # Create pages for each tag
        tags, pages = assign_entries(app)

        for tag in tags.values():
            tag.create_file(
                [item for item in pages if tag.tag in item.tags],
                app.config.tags_extension,
                tags_output_dir,
                app.srcdir,
                app.config.tags_page_title,
                app.config.tags_page_header,
            )

        # Create tags overview page
        tagpage(
            tags,
            os.path.join(app.srcdir, tags_output_dir),
            app.config.tags_overview_title,
            app.config.tags_extension,
            app.config.tags_index_head,
        )

        # Create category-specific overview pages for sidebar navigation
        category_labels = getattr(app.config, "tags_category_labels", None) or {
            "tresc": "Treści",
            "metoda": "Metody",
            "typ": "Charakter spotkania",
        }
        page_title_template = getattr(app.config, "tags_category_page_title_template", None)
        basename_template = (
            getattr(app.config, "tags_category_page_basename_template", None)
            or "kategoria-{category_key}"
        )
        outdir = os.path.join(app.srcdir, tags_output_dir)
        for category_key, label in category_labels.items():
            filtered = [t for t in tags.values() if getattr(t, "category", None) == category_key]
            tag_category_page(
                filtered,
                outdir,
                label,
                app.config.tags_extension,
                app.config.tags_index_head,
                basename_template.format(category_key=category_key),
                page_title_template=page_title_template,
            )

        logger.info("Tags updated", color="white")
    else:
        logger.info(
            "Tags were not created (tags_create_tags=False in conf.py)", color="white"
        )


def setup(app):
    """Setup for Sphinx."""

    # Create config keys (with default values)
    # These values will be updated after config-inited

    app.add_config_value("tags_create_tags", False, "html")
    app.add_config_value("tags_output_dir", "_tags", "html")
    app.add_config_value("tags_overview_title", "Tags overview", "html")
    app.add_config_value("tags_extension", ["rst"], "html")
    app.add_config_value("tags_intro_text", "Tags:", "html")
    app.add_config_value("tags_page_title", "My tags", "html")
    app.add_config_value("tags_page_header", "With this tag", "html")
    app.add_config_value("tags_index_head", "Tags", "html")
    app.add_config_value("tags_create_badges", False, "html")
    app.add_config_value("tags_badge_colors", {}, "html")
    # Optional extras for multilingual/customized installs
    app.add_config_value("tags_definition_file", None, "html")
    app.add_config_value("tags_category_labels", None, "html")
    app.add_config_value("tags_category_page_title_template", None, "html")
    app.add_config_value("tags_category_page_basename_template", None, "html")

    # internal config values
    app.add_config_value(
        "remove_from_toctrees",
        [
            app.config.tags_output_dir,
        ],
        "html",
    )

    # Update tags
    # TODO: tags should be updated after sphinx-gallery is generated, and the
    # gallery is also connected to builder-inited. Are there situations when
    # this will not work?
    app.connect("config-inited", _on_config_inited)
    app.connect("builder-inited", update_tags)
    app.add_directive("tags", TagLinks)

    return {
        "version": __version__,
        "parallel_read_safe": True,
        "parallel_write_safe": True,
        "env_version": 1,
    }

